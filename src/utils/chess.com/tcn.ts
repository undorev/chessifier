...
export function decodeTCN(moveCode: string): NormalMove {
  ...
  for (let i = 0; i < codeLength; i += 2) {
    ...
    if (pieceIndex2 > 63) {
      ...
      const newIndex = pieceIndex1 + (pieceIndex1 < 16 ? -8 : 8) + ((pieceIndex2 - 1) % 3) - 1;

      move.promotion = charToRole(promotion);
      pieceIndex2 = newIndex;
    }

    move.from = parseSquare(pieceEncoding[pieceIndex1 % 8] + (Math.floor(pieceIndex1 / 8) + 1).toString())!;

    move.to = parseSquare(pieceEncoding[pieceIndex2 % 8] + (Math.floor(pieceIndex2 / 8) + 1).toString())!;
