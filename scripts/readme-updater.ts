 ...
 */
function extractExportByFilename(filePath: string): TranslationData | undefined {
  try {
...
function calculateTranslationProgress(basePath: string, translatedPath: string): number {
  try {
    ...
    const translatedCount = baseKeys.reduce((count, key) => {
      const hasTranslation = translatedKeys.includes(key) &&
        translated[key] !== '' &&
        translated[key] !== null &&
...
function updateReadme(): void {
  try {
    const langs = fs.readdirSync('./src/translation')
    ...
    for (const lang of langs) {
      const percent = calculateTranslationProgress(
        BASE_PATH,
        `./src/translation/${lang}.ts`
      );
